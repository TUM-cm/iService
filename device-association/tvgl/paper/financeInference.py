...
def solveProblem(gvx, index_penalty, cov_mode, alpha, beta, timesteps, timeShift,
                 Cov_set, use_kernel, sigma, sample_set, empCov_set, eps_abs = 1e-4, eps_rel = 1e-4):
    if use_kernel:
        empCov_set = []
    for i in range(timesteps):
        if cov_mode == 'Syn':
...
if __name__ == "__main__":
    ...
    if set_length > 1:
        ...
        Fro_error = np.reshape(FroError,(set_length, set_length))
        Score = np.reshape(Score,(set_length, set_length))
        AIC = np.reshape(AIC,(set_length, set_length))
        FroThetaDiff = np.reshape(FroThetaDiff,(set_length, set_length))
