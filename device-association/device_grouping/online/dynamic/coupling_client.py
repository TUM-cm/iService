...
class ClientCouplingFactory(protocol.ClientFactory):
    ...
    def __init__(self, user):
        self.user = user
        self.transport = None
        self.data_provider = self.user.get_coupling_data_provider()
    
    def buildProtocol(self, addr):
        logging.info("addr: " + str(addr))
        client = ClientCouplingProtocol(self)
        self.user.set_client(client)
        self.user.start()
        return client
    ...
    def get_route(self):
        return self.user.get_route()
    
class ClientController:
    
    def __init__(self, server_ip, server_port, user):
        self.server_ip = server_ip
        self.server_port = server_port
        self.factory = ClientCouplingFactory(user)
        certData = utils.ssl.get_public_cert()
        authority = ssl.Certificate.loadPEM(certData)
        self.options = ssl.optionsForClientTLS(self.server_ip, authority)
    
    def start(self):
        self.connect = reactor.connectSSL(
            self.server_ip, self.server_port, self.factory, self.options)
